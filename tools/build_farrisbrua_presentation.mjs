...
function addFooter(slide, section = "B43 øvelse") {
  ...
  slide.addText("OVELSE - fiktivt scenario", { x: 9.9, y: 7.05, w: 2.8, h: 0.18, fontFace: "Arial", fontSize: 7.5, color: C.muted, align: "right" });
}
...
// 1 cover
{
  ...
  slide.addText("Realistisk spillovelse for 110-vaktlag", { x: 0.75, y: 2.78, w: 8.7, h: 0.36, fontSize: 18, color: C.ink });
  ...
  slide.addText("Scenarioet er ukjent for de ovende ved start. Bildet brukes forst i ettergjennomgang eller pa riktig tidspunkt i spillet.", { x: 0.75, y: 6.03, w: 11.8, h: 0.28, fontSize: 10.5, color: C.muted });
...
{
  const slide = pptx.addSlide();
  title(slide, "For-brief: rammer for de ovende", "Del 1");
  bulletList(slide, [
    "B43 er en fiktiv ovelsessentral i kontrollromsetting.",
    "Ovelsen varer ca. 90 minutter og kjores som spillovelse.",
    "Systembruk/LEO evalueres bare der det er nodvendig for gjennomforingen.",
  ...
  slide.addText("STOPP OVELSE B43", { x: 7.75, y: 2.65, w: 4.2, h: 0.42, fontSize: 22, bold: true, color: C.navy, fit: "shrink" });
  slide.addText("Ved reell hendelse eller uklarhet stoppes ovelsen og ovingsleder avklarer videre handtering.", { x: 7.75, y: 3.35, w: 4.05, h: 0.75, fontSize: 12, color: C.ink, fit: "shrink" });
...
{
  const slide = pptx.addSlide();
  title(slide, "Laringsmal for vaktlaget", "Del 1");
  const rows = [
    ["1", "Handtere forste nodmelding og utalarmering strukturert og tidsriktig."],
    ["2", "Etablere og oppdatere felles situasjonsforstaelse mens ressurser er pa vei."],
    ["3", "Samvirke hensiktsmessig med politi, AMK og VTS."],
    ["4", "Fordele oppgaver og sikre overgang fra akuttfase til driftsfase."],
    ["5", "Oppsummere status presist for ovelsen avsluttes."]
  ];
  let y = 1.65;
...
// 4 forventet arbeidsform
{
  ...
  slide.addText("Dette er ikke en patvunget fasit, men en struktur som gjor det mulig a lykkes i et kunstig ovingsmiljo.", { x: 1.0, y: 5.75, w: 11.3, h: 0.3, fontSize: 12, color: C.muted, align: "center" });
  addFooter(slide, "Før-brief");
}
...
// 5 spillstab brief
{
  ...
  bulletList(slide, [
    ...
    "Ikke legg inn farlig last, sabotasje, mange hardt skadde eller storre eskalering uten ovingsleder.",
    "Innringer 2 og 3 skal bare teste håndtering av duplikat-/avstandsmeldinger.",
    "Forsvaret/militaer kolonne er kontekst, ikke hovedaktor.",
    "Dersom teknikk svikter, gis innspillet muntlig av ovingsleder."
  ], 0.85, 1.65, 6.3, 4.25, { fontSize: 14.5 });
  slide.addImage({ path: path.join(imgDir, "bilde_2_farrisbrua.png"), x: 7.65, y: 1.65, w: 4.65, h: 2.62 });
  slide.addText("Bildepakke brukes bare pa definert tidspunkt.", { x: 7.75, y: 4.55, w: 4.3, h: 0.3, fontSize: 12.5, color: C.muted, align: "center" });
...
{
  const slide = pptx.addSlide();
  title(slide, "Ettergjennomgang: faktisk ovelsesforlop", "Del 3");
  const phases = [
    ["0-8", "Nødtelefon\nutalarmering"],
    ["8-22", "Fremkjøring\nsituasjonsbilde"],
    ["22-35", "Forste ressurs\nvindusmelding"],
    ["35-75", "Driftsfase\navklaringer"],
    ["75-90", "Status\nAAR"],
  ];
  phases.forEach(([t, label], i) => {
    ...
    slide.addText(t + " min", { x: x + 0.12, y: 2.28, w: 1.55, h: 0.16, fontSize: 10, bold: true, color: C.white });
    slide.addText(label, { x: x + 0.12, y: 2.55, w: 1.65, h: 0.34, fontSize: 10.5, color: C.white, fit: "shrink" });
  });
...
// 7 AAR
{
  ...
  slide.addText("Fokus pa hva og hvorfor, ikke hvem.", { x: 1.05, y: 6.15, w: 10.8, h: 0.35, fontSize: 15, bold: true, color: C.navy, align: "center" });
  addFooter(slide, "AAR");
}
